...
public abstract class MessageView extends FrameLayout {
    ...
    /**
     * Method to set the text color that you want to use in your message.
     * @param textColor The Text Color that you want to be displayed.
     */
    public abstract  void setTextColor(@ColorInt int textColor);

    public abstract  void setTStampColor(@ColorInt int tStampColor);
    ...
    public void setSeenStatus(String status) {
        if(img_seen == null) {
            this.img_seen = (ImageView) findViewById(R.id.img_seen);
        }
        ...
        if(status.equals("yes")) {
            img_seen.setVisibility(VISIBLE);
        } else {
            img_seen.setVisibility(GONE);
        }
    }
